...
static void buttons_leds_init(bool * p_erase_bonds)
{
 ...
 static void gpio_init(void)
 {
		...
		nrf_gpio_cfg_sense_input(PIN_9, NRF_GPIO_PIN_PULLDOWN, NRF_GPIO_PIN_SENSE_LOW); 
...
int main(void)
{
    ...
    for (;;)
    {
        ...
        {
            power_manage();
						/** sd_app_evt_wait() or the SEV WFE calls are often done inside the main while loop. 
						After this call we can add code to be executed when the CPU wakes up from an event. 
						You can also run code inside interrupts connected to the event, as these will run when the specific event happens and the system wakes up.
						*/
        }
    }
}
